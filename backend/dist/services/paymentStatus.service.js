...
import { buildPaymentStatusResponse, resolveProviderTransactionIdForPayment, } from "./paymentMetadata.js";
import { applyPaddleTransactionToPayment, cancelPaddleTransactionIfPossible, fetchPaddleTransaction, inferPaddleSyncEventName, readPaymentIdFromPaddleTransaction, readProviderTransactionId, } from "./paddleTransactionSync.service.js";
export async function resolvePaymentIdFromPaddleTransaction(userId, paddleTransactionId) {
    const normalized = paddleTransactionId.trim();
    if (!normalized.startsWith("txn_")) {
        throw new Error("invalid paddleTransactionId");
    }
    const payment = await paymentRepository.selectPaymentByProviderTransactionIdForUser(normalized, userId);
    if (!payment) {
        throw new Error("payment not found");
    }
    return { ok: true, paymentId: payment.id };
}
export async function getPaymentStatus(userId, paymentId, appBaseUrl) {
    const payment = await paymentRepository.selectPaymentByIdForUser(paymentId, userId);
    if (!payment) {
        throw new Error("payment not found");
    }
    return {
...
export async function syncPaymentFromPaddle(userId, paymentId, appBaseUrl, paddleTransactionId) {
    let payment = await paymentRepository.selectPaymentByIdForUser(paymentId, userId);
    if (!payment) {
        throw new Error("payment not found");
    }
    ...
    const providerTransactionId = resolveProviderTransactionIdForPayment(payment, paddleTransactionId);
    ...
    const linkedPaymentId = readPaymentIdFromPaddleTransaction(paddleTransaction.custom_data);
    ...
    const updatedPayment = await paymentRepository.selectPaymentByIdForUser(paymentId, userId);
    if (!updatedPayment) {
        throw new Error("payment not found");
    }
    return {
...
function normalizeAbandonFailureReason(rawValue) {
...
export async function abandonPendingPayment(userId, paymentId, appBaseUrl, failureReason) {
    let payment = await paymentRepository.selectPaymentByIdForUser(paymentId, userId);
    if (!payment) {
        throw new Error("payment not found");
    }
