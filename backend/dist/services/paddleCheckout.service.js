import { getPaddleApiBaseUrl, getPaddlePriceId, } from "../config/paddle.js";
...
import { buildCheckoutUrl, buildPaymentReturnUrl, getRequiredEnv, normalizeAppBaseUrl, } from "./paymentMetadata.js";
import { getSubscriptionAmount, getTokenTopupAmountUsd, parseBillingPeriod, parseTokenTopupAmount, PREMIUM_TOKEN_TOPUP_DISCOUNT_PERCENT, } from "./paymentPricing.js";
...
async function createPaddleCheckout(input) {
    ...
    const cancelUrl = `${appOrigin.replace(/\/+$/, "")}` +
...
export async function createCheckoutSession(userId, planCode, appBaseUrl, checkoutType = "subscription", tokenAmount, billingPeriod) {
    const normalizedBillingPeriod = parseBillingPeriod(billingPeriod);
    const normalizedTokenAmount = checkoutType === "token_topup"
        ? parseTokenTopupAmount(tokenAmount)
        : undefined;
    const applyPremiumTokenDiscount = checkoutType === "token_topup" &&
        (await getEffectivePlanCodeForUser(userId)) === "premium";
    const amount = checkoutType === "subscription"
        ? getSubscriptionAmount(planCode, normalizedBillingPeriod)
        : getTokenTopupAmountUsd(normalizedTokenAmount, applyPremiumTokenDiscount);
    const payment = await paymentRepository.createPendingSubscriptionPayment({
        ...
        metadata: {
            flow: checkoutType === "subscription"
    ...
    return { ok: true, checkoutSession };
}
