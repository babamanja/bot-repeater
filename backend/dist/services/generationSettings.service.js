...
export function sanitizeGenerationSettings(input) {
    const obj = input && typeof input === "object"
        ? input
        : {};
    const tokensPerChar = toPositiveNumber(obj.tokensPerChar) ??
        DEFAULT_GENERATION_SETTINGS.tokensPerChar;
    const tokensPerQuestion = toPositiveNumber(obj.tokensPerQuestion) ??
        DEFAULT_GENERATION_SETTINGS.tokensPerQuestion;
    const questionsPerChunk = toPositiveNumber(obj.questionsPerChunk) ??
        toPositiveNumber(obj.questionsPer1000Chars) ??
        DEFAULT_GENERATION_SETTINGS.questionsPerChunk;
    const minQuestions = toPositiveInteger(obj.minQuestions) ??
        DEFAULT_GENERATION_SETTINGS.minQuestions;
    const maxQuestions = toPositiveInteger(obj.maxQuestions) ??
        DEFAULT_GENERATION_SETTINGS.maxQuestions;
    const defaultQuestions = toPositiveInteger(obj.defaultQuestions) ??
        DEFAULT_GENERATION_SETTINGS.defaultQuestions;
    const chunkSizeCharsRaw = toPositiveInteger(obj.chunkSizeChars) ??
        DEFAULT_GENERATION_SETTINGS.chunkSizeChars;
    const chunkSizeChars = Math.min(MAX_CHUNK_SIZE_CHARS, Math.max(MIN_CHUNK_SIZE_CHARS, chunkSizeCharsRaw));
    const chunkOverlapCharsRaw = toNonNegativeInteger(obj.chunkOverlapChars) ??
        DEFAULT_GENERATION_SETTINGS.chunkOverlapChars;
    const chunkOverlapChars = Math.min(Math.max(0, chunkOverlapCharsRaw), Math.min(MAX_CHUNK_OVERLAP_CHARS, chunkSizeChars - 1));
    const tokensPerChunkSummary = toPositiveNumber(obj.tokensPerChunkSummary) ??
        DEFAULT_GENERATION_SETTINGS.tokensPerChunkSummary;
    const tokensPerOcrImage = toPositiveNumber(obj.tokensPerOcrImage) ??
        DEFAULT_GENERATION_SETTINGS.tokensPerOcrImage;
    const signupBonusTokens = toNonNegativeInteger(obj.signupBonusTokens) ??
        DEFAULT_GENERATION_SETTINGS.signupBonusTokens;
    const normalizedMax = Math.max(maxQuestions, minQuestions);
    const normalizedDefault = Math.min(Math.max(defaultQuestions, minQuestions), normalizedMax);
    return {
        tokensPerChar,
        tokensPerQuestion,
        questionsPerChunk,
...
export function estimateOcrImageTokenCost(settings) {
    return Math.max(1, Math.floor(settings.tokensPerOcrImage));
}
export function estimatePdfOcrTokenCost(pagesNeedingOcr, settings) {
...
export function toPublicGenerationSettings(settings) {
    const { signupBonusTokens: _signupBonusTokens, ...publicSettings } = settings;
    return publicSettings;
}
export function estimateDocumentChunkCount(text, settings) {
    const trimmed = text.trim();
    if (!trimmed) {
        return 0;
    }
    return splitTextIntoChunks(trimmed, settings.chunkSizeChars, settings.chunkOverlapChars).length;
}
export function estimateChunkSummaryTokenCount(settings) {
    return Math.max(1, Math.floor(settings.tokensPerChunkSummary));
}
/** Tokens charged upfront when creating a document (all chunks). */
export function estimateDocumentSummarizationTokens(text, settings) {
...
export function capChunkTextForQuizEstimate(chunkText) {
...
export function estimateAllChunksQuizTokens(text, questionCount, settings) {
    const trimmed = text.trim();
    if (!trimmed) {
        const q = Number.isInteger(questionCount) && questionCount > 0 ? questionCount : 0;
        return Math.max(1, Math.floor(q * settings.tokensPerQuestion));
    }
    const pieces = splitTextIntoChunks(trimmed, settings.chunkSizeChars, settings.chunkOverlapChars);
    let total = 0;
...
export function estimateQuizTokensForChunkSourceText(sourceText, questionCount, settings) {
...
export function estimateFullUploadGenerationCost(text, questionCount, settings) {
    const summarizationTokens = estimateDocumentSummarizationTokens(text, settings);
    const quizGenerationTokens = estimateAllChunksQuizTokens(text, questionCount, settings);
    return {
        summarizationTokens,
...
export function estimateTokenCount(text, questionCount, settings) {
...
export function estimateQuestionCount(text, settings) {
    const trimmed = text.trim();
    if (!trimmed) {
