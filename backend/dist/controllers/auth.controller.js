...
function resolveBodyWithAppBaseUrl(req) {
    const raw = req.body ?? {};
    const fromBody = typeof raw.appBaseUrl === "string" ? raw.appBaseUrl.trim() : "";
    if (fromBody) {
        return { ...raw, appBaseUrl: fromBody };
    }
    const fromQuery = typeof req.query?.appBaseUrl === "string" ? req.query.appBaseUrl.trim() : "";
    if (fromQuery) {
...
export async function logInWithGoogle(req, res) {
    ...
    setRefreshCookie(res, result.refreshToken);
    const isNewUser = "isNewUser" in result.auth && result.auth.isNewUser === true;
    trackAuthEvent({
...
export async function restoreDeletedAccount(req, res) {
    const requestId = extractRequestId(req);
    const body = req.body ?? {};
    const authMethod = typeof body.idToken === "string" && body.idToken.trim().length > 0 ? "google" : "password";
...
export async function resendVerificationEmail(req, res) {
    ...
    const result = await authService.resendVerificationEmail(userId, resolveBodyWithAppBaseUrl(req));
    if (!result.ok) {
        return res.status(result.status).json({ error: result.error });
    }
    return res.status(200).json({ ok: true });
