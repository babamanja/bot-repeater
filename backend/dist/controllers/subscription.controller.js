...
function getAppBaseUrl(req) {
    const origin = typeof req.body?.appBaseUrl === "string" ? req.body.appBaseUrl.trim() : "";
    if (origin) {
        return normalizeAppBaseUrl(origin);
    }
    const queryOrigin = typeof req.query?.appBaseUrl === "string"
...
export async function resumeMySubscription(req, res) {
    const userId = getRequiredUserId(req);
    if (userId === null) {
        return sendUnauthorized(res);
    }
...
export async function createMyCheckoutSession(req, res) {
    const userId = getRequiredUserId(req);
    if (userId === null) {
        return sendUnauthorized(res);
    }
    ...
    try {
        const result = await subscriptionService.createMyCheckoutSession(userId, planCode, getAppBaseUrl(req), checkoutType ?? "subscription", req.body?.tokenAmount, billingPeriod);
        ...
        const statusCode = message.includes("invalid planCode") ||
...
export async function resolveMyPaymentReturn(req, res) {
    const userId = getRequiredUserId(req);
    if (userId === null) {
        return sendUnauthorized(res);
    }
    const paddleTransactionId = typeof req.query?.paddleTransactionId === "string"
    ...
    try {
        const result = await subscriptionService.resolveMyPaymentFromPaddleTransaction(userId, paddleTransactionId);
        return res.status(200).json({ paymentId: result.paymentId });
    }
    catch (error) {
        const message = error instanceof Error ? error.message : "failed to resolve payment";
        const statusCode = message === "payment not found"
            ? 404
...
export async function getMyPaymentStatus(req, res) {
    ...
    const paymentId = typeof req.params?.paymentId === "string"
    ...
    try {
        const result = await subscriptionService.getMyPaymentStatus(userId, paymentId, getAppBaseUrl(req));
...
export async function syncMyPaymentFromPaddle(req, res) {
    const userId = getRequiredUserId(req);
    if (userId === null) {
        return sendUnauthorized(res);
    }
    ...
    try {
        const result = await subscriptionService.syncMyPaymentFromPaddle(userId, paymentId, getAppBaseUrl(req));
        return res.status(200).json(result.payment);
    }
    catch (error) {
...
export async function abandonMyPayment(req, res) {
    const userId = getRequiredUserId(req);
    if (userId === null) {
        return sendUnauthorized(res);
    }
    ...
    try {
        const result = await subscriptionService.abandonMyPendingPayment(userId, paymentId, getAppBaseUrl(req), req.body?.reason);
        return res.status(200).json(result.payment);
    }
    catch (error) {
        const message = error instanceof Error ? error.message : "failed to abandon payment";
        const statusCode = message === "payment not found" ? 404 : 400;
        return res.status(statusCode).json({ error: message });
    }
}
