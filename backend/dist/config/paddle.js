export { TOKEN_TOPUP_AMOUNTS, isTokenTopupAmount, } from "@vocab-bot/shared/pricing";
import { isTokenTopupAmount } from "@vocab-bot/shared/pricing";
/** Paddle Billing API origins per environment. Override with `PADDLE_API_BASE_URL` if needed. */
export const PADDLE_API_BASE_URL_BY_ENV = {
    sandbox: "https://sandbox-api.paddle.com",
    live: "https://api.paddle.com",
};
/** Price IDs from the Paddle dashboard (Catalog → Prices). Not secrets — differ per environment. */
export const PADDLE_PRICE_IDS_BY_ENV = {
    sandbox: {
...
function isPaddleEnvironment(value) {
...
function assertValidPriceId(priceId, label) {
...
function subscriptionPriceKey(billingPeriod) {
    return billingPeriod === "yearly"
        ? "subscription_1_year"
        : "subscription_1_month";
}
function tokenTopupPriceKey(amount) {
    return `token_topup_${amount}`;
}
export function getPaddlePriceId(input) {
    ...
    if (input.checkoutType === "subscription") {
        ...
        return assertValidPriceId(prices[key], billingPeriod === "yearly" ? "yearly subscription" : "monthly subscription");
    }
