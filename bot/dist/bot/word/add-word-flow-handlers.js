import { attachUserToVocabPair, createPairFromPrimaryWordAndLearningText, findExistingPairsForPrimaryWord, findOrCreateVocabWord, getUserLanguageNames, } from '../../domain/vocab';
import { clearAddWordFlow, getAddWordFlowState, setAddWordFlowState, } from './add-word-flow-state';
...
function truncateLabel(text) {
    if (text.length <= BTN_LABEL_MAX)
        return text;
    return text.slice(0, BTN_LABEL_MAX - 1) + '…';
}
export async function startAddWordDialog(ctx, pool) {
...
export async function handleAddWordPickCallback(ctx, pool) {
    const uid = ctx.from?.id;
    if (!uid)
        return;
    ...
    await ctx.answerCbQuery();
    await ctx.editMessageText(botT(ctx, 'add_flow_done', {
        primary: state.primaryText,
        learning: picked.learningText,
        pairId,
    }));
}
export async function handleAddWordFlowText(ctx, pool) {
    const uid = ctx.from?.id;
    const raw = getMessageText(ctx);
    if (!uid || raw === undefined)
        return;
    if (raw.startsWith('/'))
        return;
    const state = getAddWordFlowState(uid);
    ...
    if (state.step === 'await_primary') {
        ...
        const suggestedPairs = await findExistingPairsForPrimaryWord(pool, primaryWord.id, learningLangId);
        setAddWordFlowState(uid, {
            step: 'await_learning',
    ...
    if (state.step === 'await_learning') {
        ...
        const pairId = await createPairFromPrimaryWordAndLearningText(pool, uid, state.primaryWordId, learningLangId, text, nowMs);
        clearAddWordFlow(uid);
        await ctx.reply(botT(ctx, 'add_flow_done', {
            primary: state.primaryText,
            learning: text,
            pairId,
        }));
