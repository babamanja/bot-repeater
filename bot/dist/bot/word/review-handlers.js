import { applyReviewResult, getRandomDueWordsForUser, UserLanguagesNotSetError, } from '../../domain/vocab';
import { advanceAfterCardAnswer, clearReviewSession, getReviewSession, putReviewSession, REVIEW_BATCH_SIZE, validateCurrentCard, } from './review-session-state';
import { clearAddWordFlow } from './add-word-flow-state';
import { escapeHtmlText } from '../telegraf-helpers';
import { requireUserLanguagesSetOrReply } from '../user/telegram-ui';
import { botT } from '../../i18n';
async function sendReviewCard(ctx, word, nonce) {
...
export async function runVocabReview(ctx, pool) {
    if (!ctx.from?.id)
        return;
...
async function handleReviewGrade(ctx, pool, result) {
...
export async function handleReviewKnowCallback(ctx, pool) {
    await handleReviewGrade(ctx, pool, 'know');
}
export async function handleReviewDontCallback(ctx, pool) {
    await handleReviewGrade(ctx, pool, 'dont');
}
export async function handleReviewMoreCallback(ctx, pool) {
    await ctx.answerCbQuery();
    await runVocabReview(ctx, pool);
}
export async function handleReviewCancelCallback(ctx) {
    if (ctx.from?.id)
        clearReviewSession(ctx.from.id);
    await ctx.answerCbQuery();
